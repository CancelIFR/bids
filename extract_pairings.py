...
def process_page(pdf_path: str, page_num: int) -> List[List[str]]:
    """
    Process a single page of the PDF and extract pairings.
    ...
    with pdfplumber.open(pdf_path) as pdf:
        ...
        for line in lines:
            ...
            seq_match = re.match(r'SEQ\s+(\d+)\s+(\d+)\s+OPS\s+POSN\s+([A-Z]+)\s+([A-Z]+)(?:\s+(.+))?', line)
            if seq_match:
            ...
            rpt_match = re.match(r'RPT\s+(\d{4})/(\d{4})(?:\s+(.+))?', line)
            if rpt_match and current_seq:
                ...
                if len(rpt_match.groups()) > 2 and rpt_match.group(3):
                    ...
                    date_numbers = re.findall(r'\b(\d{1,2})\b', date_part)
            ...
            if current_seq and not start_dates:
                ...
                date_line_match = re.match(r'^(?:\s*\d{1,2}\s*)+$', line.strip())
                if date_line_match:
                    date_numbers = re.findall(r'\b(\d{1,2})\b', line)
                    start_dates.extend([f"{month}/{d.zfill(2)}" for d in date_numbers if d.isdigit()])
                    continue
            
            # Check if this is a flight leg line
            # Format: 1 1/2 83 281 DFW 1015/1015 L ICN 1530/0130 15.15
            leg_match = re.match(r'(\d+)\s+(\d+)/(\d+)\s+(\d+)\s+(\d+)\s+([A-Z]{3})\s+(\d{4})/(\d{4})\s+([LD])\s+([A-Z]{3})\s+(\d{4})/(\d{4})\s+(\d+\.\d+)(?:\s+(.+))?', line)
            ...
            rls_match = re.match(r'RLS\s+(\d{4})/(\d{4})\s+(\d+\.\d+)\s+\d+\.\d+\s+\d+\.\d+\s+(\d+\.\d+)(?:\s+(.+))?', line)
            ...
            layover_match = re.match(r'([A-Z]{3})\s+(.*?)(?:\s+\d+)?\s+(\d+\.\d+)(?:\s+(.+))?', line)
...
def extract_pairings(pdf_path: str, output_csv: str, start_page: int = 7, 
    ...
    print(f"Processing {total_pages_to_process} pages with {max_workers} worker threads...")
    
    # Process pages in parallel
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        # Create a dictionary to store future: page_num for progress tracking
        future_to_page = {
            executor.submit(process_page, pdf_path, page_num): page_num
        ...
        for future in concurrent.futures.as_completed(future_to_page):
            ...
            try:
                page_pairings = future.result()
                # Filter by aircraft type if specified
                if aircraft_type:
                    page_pairings = [p for p in page_pairings if p[-1] == aircraft_type]
                all_pairings.extend(page_pairings)
                
                # Update progress
                completed += 1
                if completed % 10 == 0 or completed == total_pages_to_process:
                    print(f"Processed {completed}/{total_pages_to_process} pages ({completed/total_pages_to_process:.1%})")
